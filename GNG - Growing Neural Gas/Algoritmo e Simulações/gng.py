...
class GrowingNeuralGas:
    ...
    def fit_network(self, e_b, e_n, a_max, l, a, d, passes=1, plot_evolution=False):
        ...
        plt.clf()
        plt.title('Erro Local Acumulado')
        ...
        plt.clf()
        plt.title('Propriedades da Rede')
        plt.plot(range(len(network_order)), network_order, label='Ordem da Rede')
        plt.plot(range(len(network_size)), network_size, label='Tamanho da Rede')
        plt.legend()
